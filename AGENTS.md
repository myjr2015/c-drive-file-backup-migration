...
- 公开仓库：`https://github.com/myjr2015/c-drive-file-backup-migration`
- 远端默认使用：`origin https://github.com/myjr2015/c-drive-file-backup-migration.git`
...
- CI 查看流程：
  - `gh run list --repo myjr2015/c-drive-file-backup-migration --workflow tests --limit 5`
  - `gh run watch <run_id> --repo myjr2015/c-drive-file-backup-migration --exit-status`
  - 失败时用 `gh run view <run_id> --repo myjr2015/c-drive-file-backup-migration --log-failed` 看日志后再改。
- Windows GitHub Actions 的 stdout 编码可能不是 UTF-8；CLI 中文输出兜底必须按当前 `sys.stdout.encoding` 做安全转义，不要写死 UTF-8 再用 ASCII 解码。
- 公开发布名：中文软件名 `C盘文件备份迁移`，英文仓库名 `c-drive-file-backup-migration`，早期公开版本从 `V0.0.1` 开始。
- Windows 便携版打包：
  ...
  - 输出：`release/C盘文件备份迁移-V0.0.1-windows-portable.zip`
...
- 默认启动脚本：`启动C盘文件备份迁移.bat`，启动 `app_fluent.py`。
