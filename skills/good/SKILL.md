...
- 2026-05-19 已验证 `backup_cli.py scheduled-backup --config data/schedule.json` 可完成真实备份，并能按时间戳生成快照目录。
...
- 2026-05-19 已验证真实定时备份入口仍可用，会在配置的备份目录下生成时间戳快照。
...
- 2026-05-26 `Ai会话备份 V1.0.0` 发布前已验证本地打包：`python scripts/build_windows_release.py --skip-tests` 生成 `release/ai-session-backup-v1.0.0-windows-portable.zip`，zip 内包含 `Ai会话备份.exe`、`使用说明.txt`、`_internal/assets/app.ico`、`_internal/data/schedule.json`，并用 offscreen 启动 exe 5 秒验证可运行。GitHub Release 资产名使用英文，避免 GitHub/CLI 在 Windows 中文文件名上传时把中文资产名折坏。
- 2026-05-27 首次公开发布前清理版本和名称残留的流程已验证：发布前用 `rg` 检查源码、README、AGENTS、skills、docs、scripts、GitHub workflow 中是否还存在旧产品名、旧任务名、未公开版本号或非当前公开版本号；如果软件尚未正式发布，不保留旧任务名兼容常量和公开文案。
- 2026-05-27 GitHub Release 适合采用“中文 exe + 英文 zip 资产名”的组合：用户看到的软件名仍是 `Ai会话备份.exe`，GitHub 资产名用 `ai-session-backup-v1.0.0-windows-portable.zip`，上传后用 `gh release view ... --json assets` 复查资产名。
- 2026-05-27 V1.1.0 云端备份采用独立 `cloud_backup.py`：`backup_core.py` 保持本地备份/迁移，云端模块负责 Cloudflare R2 配置、AES-GCM 加密、sha256 对象去重、manifest-last 上传；本地测试使用内存存储，不依赖真实 R2。
- 2026-05-27 本机真实 Cloudflare R2/GitHub 操作前必须先点加载全局脚本，例如 `. 'D:\code\DaiMa\#全局登录脚本\Cloudflare_R2.ps1' -ApiPath 'D:\code\DaiMa\#全局登录脚本\api.txt' -Quiet`；软件云端页支持从脚本写入的 `CLOUDFLARE_ACCOUNT_ID`、`R2_ACCESS_KEY_ID`、`R2_SECRET_ACCESS_KEY`、`R2_ENDPOINT` 环境变量填入。
